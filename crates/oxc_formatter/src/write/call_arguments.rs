    ...
    match last.and_then(|arg| arg.as_expression()) {
        Some(last) => {
            ...
            if let Some(penultimate) = &penultimate
                && ((matches!(penultimate, Argument::ObjectExpression(_))
                    && matches!(last, Expression::ObjectExpression(_)))
                    || (matches!(penultimate, Argument::ArrayExpression(_))
                        && matches!(last, Expression::ArrayExpression(_))))
            {
                return false;
            }
